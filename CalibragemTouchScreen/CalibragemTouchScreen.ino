...
#define TITLE "Calibragem do TouchScreen"
...
void report()
{
    ...
    tft.println(F("Instrucoes!"));
    tft.println(F("Altere os seguintes valores"));
    tft.println(F("no codigo do seu projeto!"));
    tft.println(F("x = map(p.x, LEFT, RT, 0, tft.width());"));
    tft.println(F("y = map(p.y, TOP, BOT, 0, tft.height());"));
    tft.println(F("Altere p.x and p.y conforme"));
    tft.println(F("a orientacao da tela desejada!\n"));
...
{
    tft.fillScreen(BLACK);
    tft.fillRect(0, 0, dispx, 20, RED);
    tft.fillRect(0, 20, dispx, 2, WHITE);
    centerprint(s, 10);
    tft.setCursor(50, 50);
...
void startup()
{
    centertitle(TITLE);
    tft.println(F("\n"));
    tft.println(F("Use uma caneta ou algo parecido!"));
    tft.println(F("Toque em cada um"));
    tft.println(F("dos quadrados brancos"));
    tft.println(F("Segure ate ficar vermelho"));
    tft.println(F("Repita para cada um dos quadrados.\n"));
    tft.println(F("Os parametros de calibragem"));
    tft.println(F("tambem estarao disponiveis"));
    tft.println(F("na tela Serial ao finalizar"));
    tft.println(F("o processo!\n"));
    tft.println(F("Toque na tela para Iniciar!"));
